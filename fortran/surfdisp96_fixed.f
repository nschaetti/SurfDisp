...
c         Also permit one layer with fluid is base of the velocity is 0.&
c-----
...
c-----
       ...
       subroutine surfdisp96(thkm, vpm, vsm, rhom, nlayer, iflsph,
     & iwave, mode, igr, kmax, t, cg, err)
        double precision :: thkm(*), vpm(*), vsm(*), rhom(*)
        integer :: nlayer, iflsph, iwave, mode, igr, kmax
        double precision :: t(*), cg(*), err
...
c-----
        ...
        double precision d(NL)
        double precision a(NL)
        double precision b(NL)
        double precision rho(NL)
        double precision rtp(NL)
        double precision dtp(NL)
        double precision btp(NL)
c        common/modl/ d,a,b,rho,rtp,dtp,btp     
c        common/para/ mmax,llw,twopi
        integer*4 iverb(2)
        integer*4 llw
        integer*4 nsph, ifunc, idispl, idispr, is, ie
        double precision sone0, ddc0, h0, sone, ddc, h
        double precision cb(NL),c(NL)
...
c ---- phase velocity increment for searching root      
       ddc0 = 0.005
c ---- frequency increment (%) for calculating group vel. using g = dw/d&
       h0 = 0.005
...
c-----
        ...
        one=1.0d-2
        if(nsph.eq.1) call sphere(0,0,d,a,b,rho,rtp,dtp,btp,
     & mmax, llw, two)
        JMN = 1
        betmx=-1.e20
        betmn=1.e20
...
c        endif
        do 2000 ifunc=1,2
            ...
            if(ifunc.eq.2.and.idispr.le.0) go to 2000
                if(nsph.eq.1) call sphere(ifunc,1,d,a,b,rho,rtp,dtp,btp)
            ddc = ddc0
            sone = sone0
            h = h0
...
c-----
            call getsol(t1,c1,clow,dc,cm,betmx,iret,ifunc,ifirst,d,a,b)
...
c-----
            if(igr.gt.0) then
                ...
                clow = cb(k) + one*dc
                c1 = c1 -onea*dc
                call getsol(t1,c2,clow,dc,cm,betmx,iret,ifunc,ifirst,d,a,b)
c-----
...
c -----         calculate group velocity and output phase and group velo&
...
c-----
  ...
  100   continue
        ...
        subroutine getsol(t1,c1,clow,dc,cm,betmx,iret,ifunc,ifirst,d,a,b)
c-----
...
c-----
        ...
        omega=twopi/t1
        wvno=omega/c1
        del1 = dltar(wvno,omega,ifunc,d,a,b,rho,rtp,dtp,btp,mmax,llw,two)
...
c-----
 1300             call nevill(t1,c1,c2,del1,del2,ifunc,cn,d,a,b,rho,rtp)
                    c1 = cn
...
c - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
c
        subroutine sphere(ifunc,iflag,d,a,b,rho,rtp,dtp,btp,mmax,llw,two)
c-----
...
c-----
c        parameter(NL=100,NP=60)
        integer, parameter :: NL = 100
        double precision d(NL+1)
        double precision a(NL)
        double precision b(NL)
        double precision rho(NL)
        double precision rtp(NL)
...
c - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
c
        subroutine nevill(t,c1,c2,del1,del2,ifunc,cc,d,a,b,rho,rtp,dtp)
c-----
...
c-----
  ...
  900       continue
            c3 = x(1)
            wvno = omega/c3
            del3 = dltar(wvno,omega,ifunc,d,a,b,rho,rtp,dtp,btp, mmax,llw, a0,cpcq,cpy,cpz,cqw,cqx,xy,xz,wy,wz)
            nev = 2
            m = m + 1
            if(m.gt.10)m = 10
            goto 951
 ...
 1000 continue
        ...
        c3 = 0.5*(c1 + c2)
        wvno=omega/c3
        del3 = dltar(wvno,omega,ifunc,d,a,b,rho,rtp,dtp,btp,mmax,llw,two, a0,cpcq,cpy,cpz,cqw,cqx,xy,xz,wy,wz)
        return
        end
c
c - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
c
        function dltar(wvno,omega,kk,d,a,b,rho,rtp,dtp,btp,mmax,llw,twop, a0,cpcq,cpy,cpz,cqw,cqx,xy,xz,wy,wz)
